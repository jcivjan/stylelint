...
it('unknown custom syntax option', () => {
	return standalone({
		...
		.catch((error) => {
			expect(error.message).toBe('Cannot resolve custom syntax module unknown-module');
		});
});
...
it('unknown formatter option', () => {
	return standalone({
		...
		.catch((error) => {
			expect(error.message.startsWith('You must use a valid formatter option')).toBe(true);
		});
});
