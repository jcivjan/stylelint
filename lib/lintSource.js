...
module.exports = function lintSource(stylelint, options = {}) {
	...
	const getIsIgnored = stylelint.isPathIgnored(inputFilePath).catch((error) => {
		if (isCodeNotFile && error.code === 'ENOENT') return false;
	...
	return getIsIgnored.then((isIgnored) => {
		...
		const getConfig = stylelint.getConfigForFile(configSearchPath).catch((error) => {
